    ...
    >
      <div className="pb-10">
        <motion.p
          ...
          viewport={{ once: true }}
          className="text-4xl text-gray-be block pe-100 font-bold"
        >
        ...
        <div className="grid grid-cols-2 gap-5 pt-20">
          {LIST_SKILLS.slice(0, 2).map((s, i) => (
            ...
            >
              <div>
                <h2 className="font-bold text-2xl group-hover:text-white transition-colors duration-200">
                  {s.title}
                </h2>
                <p className="pt-5 text-justify">{s.description}</p>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        {!isViewAll && (
          <div className="flex justify-center mt-10">
            <button
              ...
              className="py-4 px-6 text-gray-be text-xl font-bold ring ring-gray-be rounded-full hover:text-white hover:ring-white transition-all duration-200 cursor-pointer"
            >
              All skills
            </button>
          </div>
